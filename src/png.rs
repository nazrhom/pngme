...
impl Png {
  ...
  pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
    ...
    match self.chunks.iter().position(|x| *x.chunk_type() == ct) {
        Some(chunk_pos) => Ok(self.chunks.remove(chunk_pos)),
        None => Err("nope"),
    }
  }
  ...
  pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
    let ct = ChunkType::from_str(chunk_type).ok()?;
  ...
  pub fn as_bytes(&self) -> Vec<u8> {
    let chunks: Vec<u8> = self.chunks().into_iter().flat_map(|c| c.as_bytes()).collect();
    Self::STANDARD_HEADER.to_vec().into_iter()
    .chain(chunks)
    .collect()
  }
}
  ...
  fn try_from(bytes: &[u8]) -> Result<Png> {
    ...
    if header_buffer != Png::STANDARD_HEADER {
      ...
      while !bytes.is_empty() {
        ...
        let chunk_byte_len = chunk.length() + 12;
        bytes = &bytes[chunk_byte_len as usize..];
        chunks.push(chunk);
      }
    }
